...
function productTemp(products) {
       pd1=products.title;
       pd2=products.category;
       pd3=products.brand;
      if(pd1){
         textvalue1=pd1;
         textvalue2=pd2;
         textvalue3=pd3;
         if((textvalue1.toUpperCase().indexOf(filter.value.toUpperCase()) > -1)||(textvalue2.toUpperCase().indexOf(filter.value.toUpperCase()) > -1)||(textvalue3.toUpperCase().indexOf(filter.value.toUpperCase()) > -1))
    { 
    return `
        <div class="prod" >
        <img class="pho" src="${products.thumbnail}">
        
        <h2 class="nam">${products.title}</h2>
        <button type="button" class="btn btn-primary" style="position:absolute; right:0.2vw; top:16.6vw;">Buy Now</button>
        <ul class="pr">
        <li  class="bb">brand=${products.brand}</li> 
        <li  class="bb">"${products.category}"</li>
        <li  class="bb">price=${products.price}   discount(%)=${products.discountPercentage}</li>
        <li  class="bb">stock=${products.stock}</li>
        <li class="bb">rating=${products.rating}</li>
        </ul>
    ...
    <h1 class="q">Products (${productsData.length} results)<h1>
    ${productsData.map(productTemp).join('')}
    <footer> this is the final list keep visiting for more updates...THANK YOU FOR VISITING!</footer>`;


        }

document.getElementById("as").innerHTML = `<h1 class="qt">Products (your results)<h1>
    ${productsData.map(productTemp).join('')}
    <footer> this is the final list keep visiting for more updates...THANK YOU FOR VISITING! © copyrights reserved </footer>`;
